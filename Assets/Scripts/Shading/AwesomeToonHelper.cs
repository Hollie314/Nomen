...
namespace AwesomeToon
{
    ...
    public class AwesomeToonHelper : MonoBehaviour
    {
        ...
        // State
        Vector3 posAbs;
        Dictionary<EntityId, LightSet> lightSets;
        ...
        void Init()
        {
            ...
            if (skinRenderer)
                skinRenderer.sharedMaterial = materialInstance;
        ...
        public void GetLights()
        {
            if (lightSets == null)
            {
                lightSets = new Dictionary<EntityId, LightSet>();
            }
            ...
            // Remove old lights
            List<EntityId> oldIds = new List<EntityId>(lightSets.Keys);

            foreach (EntityId id in oldIds)
            {
        ...
        void UpdateMaterial()
        {
            ...
            // Turn off remaining light slots
            while (i <= maxLights)
            {
            ...
            switch (light.type)
            {
                ...
                case LightType.Spot:
                    ...
                    float angle = Vector3.Angle(
                        light.transform.forward * -1f,
                        lightSet.dir.normalized
                    );

                    float inFront = Mathf.Lerp(
                        0f,
                        1f,
                        (light.spotAngle - angle * 2f) / lightSet.dir.magnitude
                    );
                ...
                default:
                    Debug.Log(
                        "Lighting type '" + light.type +
                        "' not supported by Awesome Toon Helper (" +
                        light.name + ")."
                    );
        ...
        float CalcAttenuation(float dist)
        {
            return Mathf.Clamp01(
                1.0f / (1.0f + 25f * dist * dist) *
                Mathf.Clamp01((1f - dist) * 5f)
            );
        }
